...
int main(void)
{
    ...
    UINT8 *fixed_buffer = malloc(3*FIXED_BUFFER_SIZE);
    UINT8 *c = fixed_buffer, *p = fixed_buffer + FIXED_BUFFER_SIZE, *k = fixed_buffer + 2 * FIXED_BUFFER_SIZE;
#else
    ...
    for (i=0; i<3; i++) {
        ...
        sc = safecrypto_create(SC_SCHEME_KEM_KYBER, i, flags);
        if (NULL == sc) {
