...
{
#ifdef USE_SAFECRYPTO_FLOAT_MP
    return (SC_MPF_EXP_ZERO == in->exponent)? 0 : in->sign;
#else
    return mpfr_sgn(in);
#endif
}
...
#ifdef USE_SAFECRYPTO_FLOAT_MP
    out->sign      = n->sign * d->sign;
    fprintf(stderr, "out->sign = %d, n->sign=%d, d->sign=%d\n", out->sign, n->sign, d->sign);

    if (SC_MPF_IS_SINGULAR(n) || SC_MPF_IS_SINGULAR(d)) {
        if (SC_MPF_EXP_NAN == n->exponent || SC_MPF_EXP_NAN == d->exponent) {
            // If the numerator or denominator are NaN then quotient is also a NaN
            out->exponent  = SC_MPF_EXP_NAN;
        }
        else if (SC_MPF_EXP_INF == n->exponent) {
            if (SC_MPF_EXP_INF == d->exponent) {
                // If numerator and denominator are infinite then the quotient is NaN
                out->exponent  = SC_MPF_EXP_NAN;
            }
            else {
                // If numerator and denominator are infinite then the quotient is NaN
        ...
        else if (SC_MPF_EXP_INF == d->exponent) {
            // A number dividied by infinite is zero
            out->exponent  = SC_MPF_EXP_ZERO;
        }
        else if (SC_MPF_EXP_ZERO == n->exponent) {
            if (SC_MPF_EXP_ZERO == d->exponent) {
                // If zero numerator is divided by zero then quotient is NaN
                out->exponent  = SC_MPF_EXP_NAN;
            }
            else {
                // If zero numerator is divided by non-zero then quotient is ZERO
                out->exponent  = SC_MPF_EXP_ZERO;
            }
        }
        else {
